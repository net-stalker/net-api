...
const DATA_TYPE: &str = "http_clients_filters";

#[derive(Debug, PartialEq, Eq, Clone)]
pub struct HttpClientsFiltersDTO {
    http_methods: Vec<String>,
    include_http_methods_mode: Option<bool>,
    endpoints: Vec<String>,
    include_endpoints_mode: Option<bool>,
    bytes_lower_bound: Option<i64>,
    bytes_upper_bound: Option<i64>,
}
impl API for HttpClientsFiltersDTO { }

impl HttpClientsFiltersDTO {
    pub fn new(
        http_methods: &[String],
        include_http_methods_mode: Option<bool>,
        endpoints: &[String],
        ...
        HttpClientsFiltersDTO {
            http_methods: http_methods.to_vec(),
            include_http_methods_mode,
    ...
    pub fn get_http_methods(&self) -> &[String] {
        self.http_methods.as_slice()
    }

    pub fn is_include_http_methods_mode(&self) -> Option<bool> {
        self.include_http_methods_mode
    }
...
impl Encoder for HttpClientsFiltersDTO {
    fn encode(&self) -> Vec<u8> {
        ...
        writer.set_field_name("include_http_methods_mode");
        match self.include_http_methods_mode {
            Some(include_protocols_mode) => {
                writer.write_bool(include_protocols_mode).unwrap();
                writer.set_field_name("http_methods");
                writer.step_in(IonType::List).expect("Error while entering an ion list");
                self.http_methods.iter().for_each(|protocol| {
...
impl Decoder for HttpClientsFiltersDTO {
    fn decode(data: &[u8]) -> Self {
        ...
        let (include_http_methods_mode, http_methods) = match binary_user_reader.current()  {
            StreamItem::Value(_) => {
...
impl Typed for HttpClientsFiltersDTO {
...
mod tests {
    ...
    use crate::api::http_clients::http_clients_filters::HttpClientsFiltersDTO;

    
    #[test]
    fn reader_correctly_read_encoded_http_clients_filters_0() {
        ...
        let filters = HttpClientsFiltersDTO::new(
            &[],
            None,
            &endpoints,
            Some(INCLUDE_ENDPOINTS_MODE),
        ...
        assert_eq!("include_http_methods_mode", binary_user_reader.field_name().unwrap());
    ...
    #[test]
    fn reader_correctly_read_encoded_http_clients_filters_1() {
        ...
        assert_eq!(StreamItem::Value(IonType::List), binary_user_reader.next().unwrap());
        assert_eq!("http_methods", binary_user_reader.field_name().unwrap());
        binary_user_reader.step_in().unwrap();
        let elements = binary_user_reader.read_all_elements().unwrap();
        assert_eq!(elements.len(), http_methods.len());
        for (element, core_protocol) in elements.iter().zip(http_methods.as_slice()) {
            let protocol = element.as_string().unwrap();
    ...
    #[test]
    fn reader_correctly_read_encoded_http_clients_filters_2() {
        let http_methods = vec!["GET".to_string(), "POST".to_string()];
        const INCLUDE_PROTOCOLS_MODE: bool = false;
        let endpoints = vec!["0.0.0.0".to_string(), "1.1.1.1".to_string()];
        const INCLUDE_ENDPOINTS_MODE: bool = true;
    ...
    #[test]
    fn endec_http_clients_filters() {
        let http_methods = vec!["GET".to_string(), "POST".to_string()];
        const INCLUDE_PROTOCOLS_MODE: bool = false;
        let endpoints = vec!["0.0.0.0".to_string(), "1.1.1.1".to_string()];
        const INCLUDE_ENDPOINTS_MODE: bool = true;
        
        let filters: HttpClientsFiltersDTO = HttpClientsFiltersDTO::new(
            &http_methods,
            Some(INCLUDE_PROTOCOLS_MODE),
            &endpoints,
            Some(INCLUDE_ENDPOINTS_MODE),
        ...
        assert_eq!(filters, HttpClientsFiltersDTO::decode(&filters.encode()));
    }
}